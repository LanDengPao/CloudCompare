...
//onstants used for fetching QVariant data from GraphModel.
    enum class NodeRole {
...
{
  return ::qHash(key.outNodeId, seed) ^ ::qHash(key.outPortIndex, seed) ^
         ::qHash(key.inNodeId, seed) ^ ::qHash(key.inPortIndex, seed);
}

inline bool operator<(ConnectionId const &a, ConnectionId const &b)
{
  return a.outNodeId < b.outNodeId  || a.inNodeId < b.inNodeId 
      || a.outPortIndex == b.outPortIndex || a.inPortIndex == b.inPortIndex;
}
