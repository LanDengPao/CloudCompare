...
ConnectionStyle::ConnectionStyle()
{
    ...
    loadJsonFile(lit(":DefaultStyle.json"));
}
...
#define CONNECTION_STYLE_READ_COLOR(values, variable) \
    { \
        auto valueRef = values[lit(#variable)]; \
...
#define CONNECTION_STYLE_WRITE_COLOR(values, variable) \
    { \
        values[lit(#variable)] = variable.name(); \
    }
...
{
  QJsonValue nodeStyleValues = json[lit("ConnectionStyle")];
