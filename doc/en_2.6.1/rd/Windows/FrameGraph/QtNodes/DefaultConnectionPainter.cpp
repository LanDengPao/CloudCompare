...
QPainterPath DefaultConnectionPainter::cubicPath(ConnectionGraphicsObject const &connection) const
{
    ...
    //return cubic;
    auto const &ret = connection.paintPath();
    return ret;
}
...
void DefaultConnectionPainter::drawHeadArrow(QPainter *painter,
                                             ConnectionGraphicsObject const &cgo) const
{
    // 箭头现在已经集成到路径中，这里可以为空实现
    // 或者保留为额外的箭头绘制选项
    Q_UNUSED(painter);
    Q_UNUSED(cgo);
}
